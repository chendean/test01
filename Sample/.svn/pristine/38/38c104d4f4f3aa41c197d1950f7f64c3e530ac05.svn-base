...
#from utils import threadeddict, iters, iterbetter, safestr, safeunicode
from web.utils import threadeddict, iters, iterbetter, safestr, safeunicode

#this is the way of web.py printting debug message,maybe we should replace it to user-defined msg
'''try:
    # db module can work independent of web.py
    from web.webapi import debug, config
except:
    import sys
    debug = sys.stderr
    config = storage()'''
...
def reparam(string_, dictionary):
    ...
    vals = []
    result = []
    #20130115 dean: 为避免sql文中含有关键字[$]，在参数集dictionary未指定的情况下不处理
    if dictionary=={}:
...
class DB:
    ...
    def query(self, sql_query, vars=None, processed=False, _test=False):
        ...
        #replace out to jsonobject
        if db_cursor.description:

            josndic = []
            rows = [x for x in db_cursor]
            if (rows==None):
                return josndic
