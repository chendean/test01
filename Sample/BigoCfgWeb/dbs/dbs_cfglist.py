...
#一览画面
class cfglist(base.WebBase):
    def get(self):

        #render画面对象
        return render.dbs_cfglist()
...
class cfgload:
    ...
    def __getRows(self,filters=None):
        ...
        for onefile in filelist:
            ...
            namedic={'filename':onefile.encode('utf-8')}
            dbsdic=xmlreader.readDbs(piece='cfginfo')
                ...
                if (groupOp=='and') or (groupOp=='AND'):
                    ...
                    for cmprules in rules:
                        ...
                        if(data=='all') or (data=='ALL'):
                            ...
                            if ('date' in field):
                                cmpresult=self.__getfilter(dbsdic[field],op,data,'date')
                            else:
                                cmpresult=self.__getfilter(dbsdic[field],op,data)
    ...
    def POST(self,op):
        ...
        #[追加]事件
        if op=='3':
            dbs={'name':'','db':None,'tabledic':None,'task':None}
